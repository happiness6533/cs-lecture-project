...
# 2. 논블로킹 방식의 코드 흐름
# os가 io 집약적인 작업이 완료되었다고 알려주는 것과 무관하게
# cpu는 자신의 계산을 계속 진행하는 방식
# 장점: 효율적이이지만
# 단점: 코드가 진행되는데 문제가 발생할 수 있다
# 해결법
# : 논블로킹 방식의 코드 흐름은 과제 중간에 엽떡이 필요한 경우 과제가 진행될 수 없다는 문제가 있다
...
# 4. async 방식의 코드
# io 작업쪽에서 작업 완료 여부를 os에게 알려주면 cpu에게 인터럽트를 걸어서 작업 완료 여부를 알려주는 방식
# 작업이 완료된 후에 알려주기 때문에 효율적이다

# 결론
# 1. 블로킹 sync
# 장성익이 하라는 과제는 안하고 엽떡이 오기를 기다리는데 + 계속 os에게 io 친구 어디쯤 왔는지 확인하는 방식

# 2. 논블로킹 sync + 코루틴을 통한 제어권 제어
# 장성익이 과제를 계속 진행하면서 + 계속 os에게 io 친구 어디쯤 왔는지 확인하는 방식
# 엽떡이 필요한 과제가 존재하는 경우 엽떡을 사 오면 그 부분만 나중에 따로 진행할 수 있도록 코루틴으로 제어

# 3. 블로킹 async
# 장성익이 하라는 과제는 안하고 엽떡이 오기를 기다리는데 + io 친구가 엽떡을 사오면 os가 장성익에게 알려준다

# 4. 논블로킹 async + 코루틴을 통한 제어권 제어(베스트 옵션)
# 장성익이 과제를 계속 진행하면서 + io 친구가 엽떡을 사오면 os가 장성익에게 알려준다
# 엽떡이 필요한 과제가 존재하는 경우 엽떡을 사 오면 그 부분만 나중에 따로 진행할 수 있도록 코루틴으로 제어
####################################################################################################
...
# cpu의 계산 속도가 메모리의 처리 속도보다 훠얼씬 더 빠르기 때문에 그 어떤 프로세스도 중지되지 않는 것 처럼 보인다
# 따라서 여러 프로세스가 동시에 일하는 것 처럼 느껴진다
